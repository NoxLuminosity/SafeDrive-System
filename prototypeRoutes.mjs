...
function determineOverallStatus(grip, eyes, bpm, spo2) {
  let abnormalCount = 0;
...
// Helper function to format timestamp with MM/DD/YY and time
...
// ✅ Log a notification whenever there's a fault detected (regardless of change)
function logFaultNotifications(newData) {
...
router.get("/prototype", (req, res) => {
  ...
  // Get notifications from both files and combine them
  ...
  // Filter out combined issues (only keep single-issue notifications)
  ...
  // Rewe duplicates and sort by timestamp (newest first)
  const uniqueNotifications = singleIssueNotifications
    ...
    .map(notif => {
      ...
      // Return the formatted timestamp instead of ISO string
      return {
...
router.post("/prototype", (req, res) => {
  ...
  // Check for each fault (always create notifications when fault exists)
  ...
  const updatedData = {
    ...
    overallStatus: determineOverallStatus(gripStatus, eyesStatus, bpm, spo2),
    notifications: allNotifications,
    updatedAt: new Date().toISOString(),
  };

  // Also add to notifications.json file
